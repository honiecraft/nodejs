...
function MainHistory(props) {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      setIsLoading(true);
      const response = await HistoryAPI.getOrders(user._id, "all");
  ...
  return (
    <div className="container">
      ...
      <div className="table-responsive pt-5 pb-5 w-100">
        {isLoading ? (
          <p>Loading...</p>
        ) : (
            ...
            <tbody>
              {orderList.map((value) => (
                <tr className="text-center" key={value._id}>
                  ...
                  <td className="align-middle border-0">
                    <p className="mb-0 small">{value.user.userId._id}</p>
                  </td>
                  <td className="align-middle border-0">
                    <p className="mb-0 small">{value.user.userId.fullname}</p>
                  </td>
                  <td className="align-middle border-0">
                    <p className="mb-0 small">{value.user.userId.phone}</p>
                  </td>
                  <td className="align-middle border-0">
                    <p className="mb-0 small">{value.user.userId.address}</p>
                  </td>
                  <td className="align-middle border-0">
